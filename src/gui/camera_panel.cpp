...
void draw_camera_panel(camera_panel_state& state, camera& cam) {
    ...
    if (cam.get_mode() == camera_mode::FOLLOW) {
        ...
        if (ImGui::SliderFloat("Min Distance", &state.min_dist, 0.5f, 10.0f, "%.1f m")) {

            cam.set_follow_distance_limits(state.min_dist, state.max_dist);
        }

        if (ImGui::SliderFloat("Max Distance", &state.max_dist, 5.0f, 30.0f, "%.1f m")) {
